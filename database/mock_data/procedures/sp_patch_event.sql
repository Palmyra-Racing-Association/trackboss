...
BEGIN
	SELECT date, event_name, event_description, event_type_id
    INTO @cur_date, @cur_event_name, @cur_event_description, @cur_event_type_id
    FROM event e
	...
	IF(@date_dif != 0) THEN
			...
			CALL sp_event_job_generation(IFNULL(_date, @cur_date), @cur_event_type_id, IFNULL(_event_name, @cur_event_name), IFNULL(_event_description, @cur_event_description));
	END IF;
